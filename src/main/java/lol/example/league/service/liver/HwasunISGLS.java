...
@Slf4j
public class HwasunISGLS {

    private final String folderPath = "/Users/yangwonjong/python-excel/Hwasun/result/";
    private final String resultPath = "/Users/yangwonjong/python-excel/Hwasun/isgls/";
...
// 2-1. 환자별 검사 날짜 정보 저장
                        if (row != null && rowindex == 0) {
                            ...
                            for (columnindex = 0; columnindex < cells; columnindex++) {
                                XSSFCell cell = row.getCell(columnindex);
                                if(columnindex>3){
                                    LocalDateTime timetest = LocalDateTime.parse(cell.getStringCellValue().replace('T',' '), formatter);
...
// 2-2. 진단 결과 데이터 읽기 시작
                        if (row != null && rowindex >= 1) {
                            // 행별 처리
                            if (row.getCell(0).getStringCellValue().equals("PT(%)") || row.getCell(0).getStringCellValue().equals("Bilirubin total")) {
...
// 2-3. pre-op 데이터 처리용 날짜 + 시간 데이터 집계(수술 전)
                                        if(dates.get(columnindex-4).isBefore(opdate.atStartOfDay()) && cell != null && !Objects.equals(cell.getStringCellValue(), "")){
                                            if(type.equals("PT(%)")){
...
// 2-4 ISGLS 비정상 검사를 위한 수술후 수치 적재 (수술후 1~4일)
                                        if(dates.get(columnindex-4).toLocalDate().isAfter(opdate) && dates.get(columnindex-4).toLocalDate().isBefore(opdate.plusDays(5))  && cell != null && !Objects.equals(cell.getStringCellValue(), "")){
                                            if(type.equals("PT(%)")){
...
// 2-5. ISGLS 검사용 항복별로 날짜별 max 수치 저장 (수술후 5~21일)
                                        if(dates.get(columnindex-4).toLocalDate().isAfter(opdate.plusDays(4)) && dates.get(columnindex-4).toLocalDate().isBefore(opdate.plusDays(22))  && cell != null && !Objects.equals(cell.getStringCellValue(), "")){
                                            if(type.equals("PT(%)")){
...
// 3. 집계한 데이터를 가지고 엑셀에 출력 작업 진행
    ...
    public String checkDataException(String data){
        if(data.contains("/")){
            ...
            log.info("data 처리 후" +  data.split("\\(")[0]);
            return data.split("\\(")[0];
        }
        else{
            return data;
