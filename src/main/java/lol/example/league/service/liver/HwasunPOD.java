...
@Slf4j
public class HwasunPOD {

    private final String folderPath = "/Users/yangwonjong/python-excel/Hwasun/pre-result/";
...
//                                            break;
                                        case "PT(%)":
                                            if (dates.get(columnindex - 4).isBefore(opdate.atStartOfDay()) && cell.getStringCellValue() != null && !Objects.equals(cell.getStringCellValue(), "")) {
                                                pre.setPT_INR(Double.valueOf(checkDataException(cell.getStringCellValue().split("/")[1])));
                                                pre.setPT_P(Double.valueOf(checkDataException(cell.getStringCellValue().split("/")[2])));
                                            }
                                            if (dates.get(columnindex - 4).toLocalDate().isAfter(opdate) && cell.getStringCellValue() != null && !Objects.equals(cell.getStringCellValue(), "")) {
//                                                String ptInr = cell.getStringCellValue().split("/")[0];
//                                                String ptPer = cell.getStringCellValue().split("/")[1];
//                                                log.info("split data " + cell.getStringCellValue().split("/")[2]);
//                                                String protien = cell.getStringCellValue().split("/")[2];
//                                                log.info("PT DATA CHECK" + cell.getStringCellValue());
                                                pod.getPT_INR().add(PatientOPDTO.PatientData.builder()
                                                        .date(dates.get(columnindex - 4))
                                                        .data(Double.valueOf(checkDataException(cell.getStringCellValue().split("/")[1])))
                                                        .build());
                                                pod.getPT_P().add(PatientOPDTO.PatientData.builder()
                                                        .date(dates.get(columnindex - 4))
                                                        .data(Double.valueOf(checkDataException(cell.getStringCellValue().split("/")[2])))
                                                        .build());
                                            }
                                            break;
                                        case "Total protein":
...
//4. 총 환자 결과 리스트를 가지고 최종 엑셀 형식으로 데이터 정렬해서 엑셀 파일 생성 진행
        try {
            File file = new File(resultPath + "pre_final_result.xlsx");
            ...
            for (PatientOPDTO.PatientPODResult set : patientResult) {
                ...
                // pt inr 처리한 내용이 순서가 바뀌었다 해서 칼럼 이름과 반대로 뒤집은 상태
                row.createCell(6).setCellValue(set.getPreop().getPT_P()!=null?set.getPreop().getPT_P():null);
                row.createCell(7).setCellValue(set.getPreop().getPT_INR()!=null?set.getPreop().getPT_INR():null);
...
//        else if(data.contains("(")){return data.split("\\(")[1].split("\\)")[0];}
        else if(data.contains("<T")){return data.split("<T")[0];}
        else if(data.contains("<")){return data.split("<")[1];}
        else if(data.contains("(재검)")){ log.info("(재검) 케이스 결과 " + data.split("\\(")[0] ); return data.split("\\(")[0];}
        else if(data.contains("[재검]")){return data.split("\\[")[0];}
        else if(data.contains("(재검결과)")){ log.info("(재검결과) 케이스 결과 " + data.split("\\(")[0] ); return data.split("\\(")[0];}
        else if(data.equals("Diff.count필요시")){return "88888";}
        else if(data.contains("?")){return data.split("\\?")[1];}
        else{
            return data;
        }
